...
if $spark; then
        if $spark_100k; then
                ...
                do
                        echo $f
                        { /usr/bin/time -f "$f, %E" /opt/ldf-client/bin/ldf-client http://curry-n1.elis.ugent.be:8000/hex_demo $f | grep "Time " ; } 2>> output_time_spark_100k.log
			sleep 10
                done
        fi
	...
	if $spark_500k; then
                ...
                do
                        echo $f
                        { /usr/bin/time -f "$f, %E" /opt/ldf-client/bin/ldf-client http://curry-n1.elis.ugent.be:8000/hex_demo $f | grep "Time " ; } 2>> output_time_spark_500k.log
                        sleep 20
                done
        fi
        ...
        if $spark_1M; then
                ...
                do
                        echo $f
                        { /usr/bin/time -f "$f, %E" /opt/ldf-client/bin/ldf-client http://curry-n1.elis.ugent.be:8000/hex_demo $f | grep "Time " ; } 2>> output_time_spark_1M.hex.log
			sleep 10
                done
        fi

        if $spark_10M; then
                if [ -f output_time_spark_10M.hex.log ]; then
                	mv output_time_spark_10M.hex.log output_time_spark_10M.hex.log.old
                fi

                for f in $QUERIES_PATHNAME_10M
                do
                        echo $f
                        { /usr/bin/time -f "$f, %E" /opt/ldf-client/bin/ldf-client http://curry-n1.elis.ugent.be:8000/hex_demo $f | grep "Time " ; } 2>> output_time_spark_10M.hex.log
                        sleep 10
                done
        fi
fi
