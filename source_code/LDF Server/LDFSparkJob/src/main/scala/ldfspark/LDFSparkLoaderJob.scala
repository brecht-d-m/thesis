...
object LDFSparkLoaderJob extends SparkSqlJob {
  ...
  def readFile(file: RDD[String], sc: SQLContext) = {
    ...
    // triples.write.mode(SaveMode.Append).parquet("hdfs://10.20.32.1:9000/tmp/triple_table_spark.parquet")
	  
    val triplesParquetFile = sc.read.parquet("triple_table_spark.parquet")
	// val triplesParquetFile = sc.read.load("hdfs://10.20.32.1:9000/tmp/triple_table_spark.parquet")
