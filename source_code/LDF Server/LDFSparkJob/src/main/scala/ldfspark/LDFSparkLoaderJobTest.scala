...
object LDFSparkLoaderJobTest extends SparkSqlJob {
  ...
  def readFile(file: RDD[String], sc: SQLContext) = {
    ...
    triples.write.mode(SaveMode.Overwrite).parquet("hdfs://172.18.16.126:9000/tmp/triple_table_spark.parquet")
    
    val triplesParquetFile = sc.read.parquet("hdfs://172.18.16.126:9000/tmp/triple_table_spark.parquet")
