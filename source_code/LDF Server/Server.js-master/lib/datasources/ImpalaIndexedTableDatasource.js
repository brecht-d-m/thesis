...
function ImpalaIndexedTableDatasource(options) {
    if(!(this instanceof ImpalaIndexedTableDatasource))
        return new ImpalaIndexedTableDatasource(options);
...
ImpalaIndexedTableDatasource.prototype._executeQuery = function (query, tripleStream, metadataCallback) {
...
function parseTriple(triple) {
    ...
    subjectIndex = parseInt(triple.subjectindex);
    if(subjectIndex == 0) {
        ...
        // Get prefix + concat
        tripleResult["subject"] = prefixArray[subjectIndex - 1] + triple.subject.trim() + ">";
        tripleResult["subject"] = tripleResult["subject"].substring(1, tripleResult["subject"].length - 1);
    }
	
    predicateIndex = parseInt(triple.predicateindex);
    if(predicateIndex == 0) {
        ...
        tripleResult["predicate"] = prefixArray[predicateIndex - 1] + triple.predicate.trim() + ">";
        tripleResult["predicate"] = tripleResult["predicate"].substring(1, tripleResult["predicate"].length - 1);
    }
	
    objectIndex = parseInt(triple.objectindex);
    if(objectIndex == 0) {
        ...
        tripleResult["object"] = prefixArray[objectIndex - 1] + triple.object.trim() + ">";
        tripleResult["object"] = tripleResult["object"].substring(1, tripleResult["object"].length - 1);
    }
...
ImpalaIndexedTableDatasource.prototype._doTotalCount = function(query, metadataCallback) {
...
ImpalaIndexedTableDatasource.prototype._createTriplePattern = function (triple, count) {
    if(count) {
		var query = "SELECT count(*) as count FROM temp_index_table ";
	} else {
		var query = "SELECT subjectIndex, subject, predicateIndex, predicate, objectIndex, object FROM temp_index_table ";
	}
...
function parseURI(uri) {
    var idx = 0;
    for(var prefix in prefixArrayTrimmed) {
        if(uri.indexOf(prefixArrayTrimmed[prefix]) === 0) {
            return [uri.substring(prefixArrayTrimmed[prefix].length, uri.length), idx+1];
        }
        idx += 1;
    }
...
module.exports = ImpalaIndexedTableDatasource;
